...
const LINK_RENDER_MODES = ["Straight", "Linear", "Spline"]
const STRAIGHT_LINK = 0
const LINEAR_LINK = 1
const SPLINE_LINK = 2

const NORMAL_TITLE = 0
const NO_TITLE = 1
const TRANSPARENT_TITLE = 2
const AUTOHIDE_TITLE = 3

const CONNECTION_DIRECTS = { 
    UP: 1,
    DOWN: 2,
    LEFT: 3,
    RIGHT: 4,
...
function LGraphCanvas(canvas, graph, options) {
    ...
    this.title_text_font = "" + NODE_TEXT_SIZE + "px Arial";
    ...
    this.links_render_mode = SPLINE_LINK;
...
LGraphCanvas.prototype.drawFrontCanvas = function() {
    ...
    if (this.graph) {
        ...
        if (this.connecting_pos != null) {
            ...
            {
                if (this.connecting_output)
                    connDir = this.connecting_node.horizontal ? CONNECTION_DIRECTS.DOWN : CONNECTION_DIRECTS.RIGHT;
                else
                    connDir = this.connecting_node.horizontal ? CONNECTION_DIRECTS.UP : CONNECTION_DIRECTS.LEFT;
            }
            ...
            this.renderLink(
                ...
                link_color,
                connDir,
                CONNECTION_DIRECTS.CENTER
            );

            ctx.beginPath();
            if (
                connType === EVENT ||
                connShape === SHAPES.BOX_SHAPE
            ) {
                ctx.rect(
            ...
            } else if (connShape === SHAPES.ARROW_SHAPE) {
...
LGraphCanvas.prototype.drawSubgraphPanelLeft = function (subgraph, subnode, ctx) {
    ...
    var w = 200;
    var h = Math.floor(NODE_SLOT_HEIGHT * 1.6);
...
LGraphCanvas.prototype.drawNode = function(node, ctx) {
    ...
    var shape = node._shape || SHAPES.BOX_SHAPE;
    var size = temp_vec2;
    ...
    if (node.clip_area) {
        ...
        ctx.save();
        ctx.beginPath();
        if (shape == SHAPES.BOX_SHAPE) {
            ctx.rect(0, 0, size[0], size[1]);
        } else if (shape == SHAPES.ROUND_SHAPE) {
            ctx.roundRect(0, 0, size[0], size[1], [10]);
        } else if (shape == SHAPES.CIRCLE_SHAPE) {
            ctx.arc(
                size[0] * 0.5,
                size[1] * 0.5,
                size[0] * 0.5,
    ...
    if (!node.flags.collapsed) {
        ...
        if (node.inputs) {
            for (var i = 0; i < node.inputs.length; i++) {
                ...
                if (slot_type == "array"){
                    slot_shape = SHAPES.GRID_SHAPE; // place in addInput? addOutput instead?
                }
                ...
                } else if (slot_shape === SHAPES.ARROW_SHAPE) {
                    ctx.moveTo(pos[0] + 8, pos[1] + 0.5);
                    ctx.lineTo(pos[0] - 4, pos[1] + 6 + 0.5);
                    ctx.lineTo(pos[0] - 4, pos[1] - 6 + 0.5);
                    ctx.closePath();
                } else if (slot_shape === SHAPES.GRID_SHAPE) {
                ...
                if (render_text) {
                    ...
                    if (text) {
                        ...
                        if (horizontal || slot.dir == CONNECTION_DIRECTS.UP) {
        ...
        if (node.outputs) {
            for (var i = 0; i < node.outputs.length; i++) {
                ...
                if (render_text) {
                    ...
                    if (text) {
                        ctx.fillStyle = NODE_TEXT_COLOR;
                        if (horizontal || slot.dir == CONNECTION_DIRECTS.DOWN) {
        ...
        if (input_slot) {
            ...
            } else if (slot.shape === SHAPES.ARROW_SHAPE) {
                ctx.moveTo(x + 8, y);
                ctx.lineTo(x + -4, y - 4);
                ctx.lineTo(x + -4, y + 4);
                ctx.closePath();
...
LGraphCanvas.prototype.drawNodeShape = function(
    ...
    var shape =
        node._shape || node.constructor.shape || SHAPES.ROUND_SHAPE;

    var title_mode = node.constructor.title_mode;

    var render_title = true;
    if (title_mode == TRANSPARENT_TITLE || title_mode == NO_TITLE) {
        render_title = false;
    ...
    {
        ctx.beginPath();
        if (shape == SHAPES.BOX_SHAPE || low_quality) {
            ctx.fillRect(area[0], area[1], area[2], area[3]);
        } else if (
            shape == SHAPES.ROUND_SHAPE ||
            shape == SHAPES.CARD_SHAPE
        ) {
            ctx.roundRect(
                area[0],
                area[1],
                area[2],
                area[3],
                shape == SHAPES.CARD_SHAPE ? [this.round_radius,this.round_radius,0,0] : [this.round_radius] 
            );
        } else if (shape == SHAPES.CIRCLE_SHAPE) {
            ctx.arc(
                size[0] * 0.5,
    ...
    if (render_title || title_mode == TRANSPARENT_TITLE) {
        ...
        if (node.onDrawTitleBar) {
            ...
            } else if (  shape == SHAPES.ROUND_SHAPE || shape == SHAPES.CARD_SHAPE ) {
                ctx.roundRect(
                    0,
                    -title_height,
                    size[0] + 1,
    ...
    if (selected) {
        ...
        if (shape == SHAPES.BOX_SHAPE) {
            ...
            shape == SHAPES.ROUND_SHAPE ||
            (shape == SHAPES.CARD_SHAPE && node.flags.collapsed)
        ) {
            ctx.roundRect(
                -6 + area[0],
                -6 + area[1],
        ...
        } else if (shape == SHAPES.CARD_SHAPE) {
            ctx.roundRect(
                -6 + area[0],
                -6 + area[1],
                12 + area[2],
...
LGraphCanvas.prototype.drawConnections = function(ctx) {
    ...
    for (var n = 0, l = nodes.length; n < l; ++n) {
        ...
        for (var i = 0; i < node.inputs.length; ++i) {
            ...
            var start_dir =
                start_slot.dir ||
                (start_node.horizontal ? CONNECTION_DIRECTS.DOWN : CONNECTION_DIRECTS.RIGHT);
            var end_dir =
                end_slot.dir ||
                (node.horizontal ? CONNECTION_DIRECTS.UP : CONNECTION_DIRECTS.LEFT);
...
LGraphCanvas.prototype.renderLink = function(
    ...
    start_dir = start_dir || CONNECTION_DIRECTS.RIGHT;
    end_dir = end_dir || CONNECTION_DIRECTS.LEFT;
    ...
    for (var i = 0; i < num_sublines; i += 1) {
        ...
        if (this.links_render_mode == SPLINE_LINK) {
            ...
            var end_offset_y = 0;
            switch (start_dir) {
                case CONNECTION_DIRECTS.LEFT:
                    start_offset_x = dist * -0.25;
                    break;
                case CONNECTION_DIRECTS.RIGHT:
                    start_offset_x = dist * 0.25;
                    break;
                case CONNECTION_DIRECTS.UP:
                    start_offset_y = dist * -0.25;
                    break;
                case CONNECTION_DIRECTS.DOWN:
                    start_offset_y = dist * 0.25;
                    break;
            }
            switch (end_dir) {
            ...
            var end_x = b[0];
            var end_y = b[1];
            if (start_dir == CONNECTION_DIRECTS.RIGHT) {
                start_x += 10;
            } else {
                start_y += 10;
            }
            if (end_dir == CONNECTION_DIRECTS.LEFT) {
                end_x -= 10;
            } else {
                end_y -= 10;
            }
    ...
    if (
        ...
        end_dir != CONNECTION_DIRECTS.CENTER
    ) {
        //render arrow
...
LGraphCanvas.prototype.processNodeWidgets = function(
    ...
    for (var i = 0; i < node.widgets.length; ++i) {
        ...
        switch (w.type) {
            case "button":
                if (event.type === pointerevents_method+"down") {
                    if (w.callback) {
                        setTimeout(function() {
            ...
            case "combo":
                var old_value = w.value;
                if (event.type == pointerevents_method+"move" && w.type == "number") {
                    if(deltaX)
                ...
                } else if (event.type == pointerevents_method+"down") {
                    var values = w.options.values;
                ...
                else if(event.type == pointerevents_method+"up" && w.type == "number")
                {
                    var delta = x < 40 ? -1 : x > widget_width - 40 ? 1 : 0;
...
LGraphCanvas.showMenuNodeOptionalOutputs = function(
    ...
    if (LiteGraph.do_add_triggers_slots){ //canvas.allow_addOutSlot_onExecuted
        if (node.findOutputSlot("onExecuted") == -1){
            entries.push({content: "On Executed", value: ["onExecuted", EVENT, {nameLocked: true}], className: "event"}); //, opts: {}
        }
    }
...
LGraphCanvas.prototype.createDefaultNodeForSlot = function(optPass) { // addNodeMenu for connection
    ...
    var fromSlotType = slotX.type==EVENT?"_event_":slotX.type;
...
LGraphCanvas.prototype.showSearchBox = function(event, options) {
    ...
    if (options.do_type_filter){
        if (selIn){
            ...
            if (options.type_filter_in == EVENT || options.type_filter_in == ACTION)
        ...
        if (selOut){
            ...
            if (options.type_filter_out == EVENT || options.type_filter_out == ACTION)
        ...
        if (that.onSearchBox) {
            ...
            function inner_test_filter( type, optsIn )
            {
                ...
                if(options.do_type_filter && !opts.skipFilter){
                    ...
                    //if (sV.toLowerCase() == "_event_") sV = EVENT; // -1
...
LGraphCanvas.prototype.showShowGraphOptionsPanel = function(refOpts, obEv, refMenu, refMenu2){
    ...
    function inner_refresh(){
        ...
        panel.addWidget( "combo", "Render mode", LINK_RENDER_MODES[graphcanvas.links_render_mode], {key: "links_render_mode", values: LINK_RENDER_MODES}, fUpdate);
...
LGraphCanvas.onMenuNodeColors = function(value, options, e, menu, node) {
    ...
    function inner_clicked(v) {
        ...
        var color = v.value ? node_colors[v.value] : null;
